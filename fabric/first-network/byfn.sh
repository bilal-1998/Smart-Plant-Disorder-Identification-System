...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    ...
    for PEER in peer0.org1.example.com peer1.org1.example.com peer2.Org1.example.com; do
...
function generateChannelArtifacts() {
  ...
  if [ "$CONSENSUS_TYPE" == "solo" ]; then
    configtxgen -profile OneOrgsOrdererGenesis -channelID byfn-sys-channel -outputBlock ./channel-artifacts/genesis.block
